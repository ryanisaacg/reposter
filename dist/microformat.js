...
export function parseMicroformat(html, uri) {
    const microformat = mf2(html, { baseUrl: uri });
    const hEntry = microformat.items.find((item) => item.type?.includes("h-entry"));
    if (hEntry == null) {
        return null;
    }
    const { author: authorCard, name: title, published: date, url: permalink, content, } = hEntry.properties;
    const { name: author, url: authorUrl, photo: avatar, } = authorCard
...
function isPropertyRoot(property) {
